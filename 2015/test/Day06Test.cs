...
namespace _2015.test
{
    ...
    public class Day6Test
    {
        ...
        public void part1_examples()
        {
            ...
            var nbLit = Part1(10, new List<Day6.Instruction>(){Day6.Instruction.FromString("turn on 0,0 through 2,2")});
            Assert.AreEqual(9,nbLit);
        }

        [Test]
        public void answer()
        {
            var instructions = data.Split(Environment.NewLine).Select(Day6.Instruction.FromString).ToList();
            var answerPart1 = Part1(1000, instructions);
            var answerPart2 = Part2(1000, instructions);
            Console.WriteLine(answerPart1);
            Console.WriteLine(answerPart2);
        }

        private string data = @"turn on 887,9 through 959,629
